...
* **多模态与富文本并发**：支持批量图片并发 OCR 解析，并能隐式抓取并解析笔记中包含的网页链接。
* **实体与知识图谱构建**：深入分析文本脉络，显式抽取“实体 (Entities)”及其连带的“关系 (Relations)”，编织你的私人知识网络。
...
```mermaid
graph TD
    A[Client Interface] --> B[Node.js Orchestrator]
    B --> C[Capsule Engine]
    C --> D[Vector Database]
    C --> E[Local LLM]
    B --> F[Privacy Guard / Context Filter]
    F -.-> G[(Optional Cloud LLM)]
...
* **Phase 1 — 记忆基石 (已完成)**：统一多模态富文本胶囊（笔记/图片/网页并发解析）、本地化视觉大模型管道（VLM Pipeline）。
* **Phase 2 — 智能架构 (已完成)**：显式知识图谱引擎（实体与关系表、提及计数）、时间轴视图重构、基于向量的混合搜索。
* **Phase 3 — 知识演进 (进行中)**：图谱动态治理、自进化 AI 记忆梳理、更深度的交互式动作提取。
